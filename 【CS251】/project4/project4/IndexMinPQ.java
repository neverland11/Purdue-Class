...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    ***************************************************************************/
    private boolean greater(int i, int j) {
        if(keys[pq[i]].compareTo(keys[pq[j]]) == 0) { //the popular is the same
            if (((Song) keys[pq[i]]).getDate() < ((Song) keys[pq[j]]).getDate()){
                return false; //i(parent) is older, so not change
            } else if (((Song) keys[pq[i]]).getDate() > ((Song) keys[pq[j]]).getDate()){
                return true; //i(parent) is newer, so change
            }
        }
